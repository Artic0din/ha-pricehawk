...
def main() -> int:
    ...
    md += [
        ...
        "## Interpretation",
        "",
        f"- If TOTAL gate is PASS: refactor can proceed; new evaluator is parity-equivalent to legacy at the algorithm level.",
        f"- If TOTAL is FAIL but per-day diffs are random ±X: likely a numerical-precision quirk; investigate but probably acceptable.",
        f"- If a SPECIFIC day fails (e.g. ZEROHERO 'lost' day shows large diff): incentive parser logic divergence between legacy ZeroHeroTracker (instantaneous threshold) and new evaluator parser (avg-over-window threshold). May require switching new parser to instantaneous logic or sub-sample driver for legacy parity.",
        "",
