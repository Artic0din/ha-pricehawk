...
def _capture_reset_today_handler(hass: MagicMock) -> Any:
...
def test_handle_reset_today_raises_HAE_when_no_entries():
    ...
    try:
        ...
        assert "no PriceHawk entries" in str(exc).lower() or "reload" in str(exc).lower()
    else:
