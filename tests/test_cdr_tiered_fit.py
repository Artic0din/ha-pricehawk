...
class TestParseRateFirstDialect:
    ...
    def test_alinta_stepped_fit_exact_text(self):
        ...
        text = (
            "This Energy Plan includes a stepped feed-in tariff, where "
    ...
    def test_origin_period_averaged(self):
        ...
        text = (
            "Origin offers 12 cents per kWh until a daily export limit "
    ...
    def test_energyaustralia_solar_max(self):
        ...
        text = (
            "Solar Max is for electricity only and is available to "
...
class TestParseQuantityFirstDialect:
    ...
    def test_agl_tiered_fit_exact_text(self):
        ...
        text = (
            "This plan features a tiered feed-in tariff. For the first "
...
class TestParseEdgeCases:
    ...
    def test_unrelated_disclaimer_returns_none(self):
        text = (
            "The Terms and Conditions for Feed-in Tariffs - Victoria "
...
class TestApplyDayCap:
    ...
    def test_agl_single_day_above_cap_with_tier2(self):
        ...
        rule = {
            "tier1_c_per_kwh": Decimal("6"),
            "cap_kwh": Decimal("10"),
            "tier2_c_per_kwh": Decimal("1.5"),
            "cap_window": "DAY",
            "source": "test",
        }
...
class TestApplyPeriodCap:
    ...
    def test_origin_period_pool_exhausted_early(self):
        ...
        slots = _slots({f"2026-05-{day:02d}": [30.0] for day in range(1, 11)})
        # Pad to full 30-day period so effective_cap = 8 × 30 = 240
        for day in range(11, 31):
            slots.append({"ts_local": f"2026-05-{day:02d}T09:00:00", "grid_export_kwh": 0.0})
        b = _StubBreakdown()
        apply_rule(rule, slots, b, base_fit_c_per_kwh=Decimal("0.04"))
...
class TestParseFromIncentives:
        ...
        incentives = [
            {
                ...
                "eligibility": (
                    "This Energy Plan includes a stepped feed-in "
        ...
        incentives = [
            {
                ...
                "description": (
                    "This plan features a tiered feed-in tariff. "
    ...
    def test_returns_first_match_when_multiple_present(self):
        incentives = [
            ...
            {"displayName": "Tiered FiT", "eligibility": "7c/kWh for the first 10 kWh"},
        ]
        rule = parse_from_incentives(incentives)
        assert rule is not None
        assert rule["tier1_c_per_kwh"] == Decimal("7")
