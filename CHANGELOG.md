...
### Refactored (Constitution P14 — systemic > local)
...
- **Storage migration policy + scaffolding for safe storage version bumps.** Engineering Constitution P16 (Data Integrity). Lays the migrator machinery so a future deliberate `STORAGE_VERSION` bump no longer triggers the prior silent-discard behaviour in `coordinator.async_restore_state` — which would have wiped every user's accumulated `daily_cost_history`, monthly savings, ranked alternatives, and provider accumulators on the first bump. New `custom_components/pricehawk/storage.py` subclasses HA's `Store` and supplies the `_async_migrate_func` callback HA invokes on version mismatch; new `async_migrate_entry` in `__init__.py` covers the config-entry version axis. Both walk per-version migrators sequentially, refuse downgrades, and raise loudly on missing migrators. `async_migrate_entry` raises `ConfigEntryError` (Constitution P19 — HA platform convention) so the diagnostic message lands in the UI instead of a generic "needs migration" notice. Adds `STORAGE_MINOR_VERSION` and `CONFIG_ENTRY_VERSION` constants. Migration policy documented in `docs/architecture.md` § "Storage migration policy". (`custom_components/pricehawk/storage.py`, `custom_components/pricehawk/__init__.py`, `custom_components/pricehawk/const.py`, `custom_components/pricehawk/coordinator.py`, `docs/architecture.md`)

### Changed

- **`STORAGE_VERSION` bumped 1 → 2 with a no-op identity migrator.** Validates the migrator chain end-to-end against every existing install (all on v1) BEFORE a real schema change relies on it. The identity migrator (`storage._v1_to_v2_no_op`) round-trips the payload unchanged and stamps `_storage_version = 2`; if the chain is broken, this catches it cheaply rather than during a substantive bump. (`custom_components/pricehawk/const.py`, `custom_components/pricehawk/storage.py`)
- **`coordinator.async_restore_state` version-sentinel branch narrowed to the legacy/corrupt case.** The `PriceHawkStore` migrator already guarantees the envelope is at current version by the time `async_load` returns, so the in-payload `_storage_version` check below is only reachable when the payload itself lacks the field — pre-Phase-2 unversioned writes, partial disk writes, or hand-edited `.storage` files. Log message updated to call out that specific case; no behaviour change. (`custom_components/pricehawk/coordinator.py`)
...
### Fixed (Codex follow-up #2 on PR #169)

- **Config-entry minor-version migrator chain added.** The prior implementation only advanced on MAJOR; a same-major minor upgrade (1.1 → 1.2) would slip through migration entirely and the legacy minor body would be silently stamped at the new minor. Added `_CONFIG_ENTRY_MINOR_MIGRATORS` registry (placeholder, empty at 1.1) and a second loop in `async_migrate_entry` that walks minors within the current major after the major chain finishes. Same loud-failure contract as the major chain — missing migrator raises `ConfigEntryError`. Constitution P16. (`custom_components/pricehawk/__init__.py`)
- **`async_update_entry` now stamps the TARGET minor instead of hard-coded 1.** The previous call passed `minor_version=1` regardless of how far the migrator walked. A 1.0 → 1.5 migration would have left the entry stamped at 1.0/1.1, so the next load would re-enter migration and re-run every minor migrator. Now reads `CONFIG_ENTRY_MINOR_VERSION` from `const.py`. (`custom_components/pricehawk/__init__.py`)
- **Downgrade guard now catches same-major newer-minor.** The prior guard only fired on a newer MAJOR; a 1.2 entry loaded by a 1.1 integration would fall through, the (empty) migrator walks would no-op, then we'd stamp the entry at 1.1 and HA would persist it — silent forward-minor downgrade. New branch raises `ConfigEntryError` with a "refusing to downgrade minor" message. (`custom_components/pricehawk/__init__.py`)
- **`CONFIG_ENTRY_MINOR_VERSION` constant** added to `const.py` alongside the existing `CONFIG_ENTRY_VERSION` under the same two-tier policy as `STORAGE_VERSION`/`STORAGE_MINOR_VERSION`. Pinned to 1 at the current schema. (`custom_components/pricehawk/const.py`)
- **3 new tests** in `tests/test_runtime_data.py`: `test_async_migrate_entry_runs_minor_chain_when_major_matches` (m1→m2→m3 chain with sentinels proving step 2 sees the m1→m2 output, not a re-read), `test_async_migrate_entry_persists_target_minor_version` (asserts the patched `CONFIG_ENTRY_MINOR_VERSION=2` lands in the `async_update_entry` kwargs), `test_async_migrate_entry_rejects_newer_minor_same_major` (asserts `ConfigEntryError` raises when `entry.minor_version > CONFIG_ENTRY_MINOR_VERSION` at the current major). (`tests/test_runtime_data.py`)
