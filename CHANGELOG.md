...
- **Behavioural coverage for the per-tick `build_explanation` rebuild gate (Constitution P17).** `coordinator.py` runs `if self._providers: build_explanation(...)` at the tail of every `_async_update_data` tick so the dashboard reflects current provider snapshots instead of the midnight rollover capture. The branch shipped without a direct test: a regression that dropped the gate would silently call `build_explanation({})` every 30 seconds and clobber `_last_explanation` with the "no data" placeholder. Added five tests in `tests/test_coordinator.py::TestPerTickBuildExplanationGating` — `test_gate_mirror_matches_production_source` (substring guard against the production gate's source so the in-test mirror cannot drift), `test_async_update_data_skips_build_explanation_when_no_providers` (empty `_providers` → `build_explanation` not called and `_last_explanation` preserved), `test_async_update_data_calls_build_explanation_each_tick_when_providers_present` (two ticks → two calls, neither memoised nor deduped, providers block matches `_build_providers_block` output, `avg_amber_spot_c_kwh=None` when Amber absent), `test_async_update_data_explanation_reflects_current_amber_spot` (mutating `amber.import_kwh_today` / `import_cost_today_c` between ticks recomputes `avg_spot` rather than caching the first value), and `test_gate_avg_spot_none_when_amber_kwh_zero` (defensive div-by-zero guard at `import_kwh_today > 0` on freshly-restarted integrations). The tests mirror the gate body rather than awaiting `_async_update_data` because `DataUpdateCoordinator` resolves to a `_MockModule` proxy under `conftest`, the same constraint that drove `TestBackfillStatusSensor` and `TestPeriodRollupSensorSmoke` (test_review_improvements.py) to mirror property bodies in-place. (`tests/test_coordinator.py`)
