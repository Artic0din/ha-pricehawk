...
- **Storage migration policy — `async_migrate_entry` + `PriceHawkStore._async_migrate_func`.** Engineering Constitution P16 (Data Integrity) — the coordinator had pinned `STORAGE_VERSION = 1` since Phase 1.x despite many additive persisted fields shipping in subsequent phases (`_external_stats_cumulative`, `_backfill_status` / `_backfill_last_run_at` / `_backfill_days_loaded` / `_backfill_plans_replayed` / `_backfill_error`, `_cheap_ranked_alternatives`, `_ranking_plan_cache`, `_external_stats_backfill_done`). `coordinator.async_restore_state` silently DISCARDED state on version mismatch — meaning the FIRST deliberate bump would have wiped every user's accumulated daily_cost_history, monthly savings, ranked alternatives, and provider accumulators. New `custom_components/pricehawk/storage.py` subclasses HA's `Store` and supplies the migration callback HA invokes on version mismatch; new `async_migrate_entry` in `__init__.py` handles the config-entry version axis. Both walk per-version migrators sequentially, refuse downgrades, and raise loudly on missing migrators rather than silently succeeding. Adds `STORAGE_MINOR_VERSION` and `CONFIG_ENTRY_VERSION` constants. Two regression tests in `test_runtime_data.py` (`test_async_migrate_entry_handles_unknown_version`, `test_store_migration_preserves_known_fields`) prove identity round-trips preserve every known field and that downgrade attempts raise instead of corrupting data. Migration policy documented in `docs/architecture.md` § "Storage migration policy". (`custom_components/pricehawk/storage.py`, `custom_components/pricehawk/__init__.py`, `custom_components/pricehawk/const.py`, `custom_components/pricehawk/coordinator.py`, `docs/architecture.md`, `tests/test_runtime_data.py`, `tests/conftest.py`)
